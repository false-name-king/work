...
const PersonItem = memo(({ id }: PersonItemProps) => {
  ...
  const handleAttendanceChange = (type: AttendanceStatus) => {
    updatePerson(id, {
      ...
      batches: type === '出勤' ? item.batches || 0 : 0,
      pieces: type === '出勤' ? item.pieces || 0 : 0,
    });
  };

  const handleToggleWorkStatus = (status: string) => {
    const next = item.workStatus.includes(status) ? item.workStatus.filter((s) => s !== status) : [...item.workStatus, status];
  ...
  return (
    <article
      ...
      className={cn('clay-card overflow-hidden transition-all duration-200 bg-white', 'border-2 border-[#D5CEBF] hover:border-[#7379E6]/60', getCardTopStripe(item.attendance))}
    >
      {/* Header Info */}
      <div className="bg-[#FAF7F2] px-3.5 py-2.5 flex flex-wrap items-center justify-between gap-2 border-b border-[#F0EBE1]">
        ...
        <div className="flex gap-1.5 items-center">
          {/* Role selector */}
          <Select value={item.role} onValueChange={(v) => updatePerson(id, { role: v })}>
            <SelectTrigger
          ...
          <div className="flex items-center gap-1">
            <Select value={item.machine?.toString() || ''} onValueChange={(v) => updatePerson(id, { machine: v ? parseInt(v, 10) : null })}>
              <SelectTrigger
              ...
              <SelectContent className="rounded-2xl border-white shadow-xl bg-[#FAF7F2]">
                {MACHINES.map((m) => (
                  <SelectItem key={m} value={m.toString()} className="font-bold text-xs">
                    {m} 号
                  </SelectItem>
                ))}
              </SelectContent>
      ...
      <div className="p-3.5 space-y-3">
        ...
        <fieldset className="p-0 m-0 border-none">
          ...
          <div role="radiogroup" aria-label={`${item.name}的出勤状态`} className="clay-tray p-1 rounded-2xl flex gap-1">
            {ATTENDANCE_TYPES.map((type) => {
              ...
              return (
                <button
                  ...
                  className={cn('flex-1 py-1.5 text-[11px] rounded-xl transition-all duration-150 active:scale-95 text-center cursor-pointer', attendanceBadgeClass(type, active))}
                >
                  {type}
                </button>
              );
        ...
        {item.attendance === '出勤' ? (
          <div className="space-y-3 pt-0.5">
            {/* Work Status Grid */}
            <div>
              <span className="text-[10px] font-black text-[#5B60C4] uppercase tracking-wider block mb-1.5">岗位类型</span>
              <div className="grid grid-cols-3 gap-1.5" role="group" aria-label={`${item.name}的岗位选择`}>
                  ...
                  return (
                    ...
                    >
                      <Checkbox
                        checked={isChecked}
                        tabIndex={-1}
                        className={cn('h-3.5 w-3.5 mb-1 pointer-events-none rounded-md', isChecked && 'border-[#7379E6] bg-[#7379E6] text-white')}
                        aria-hidden="true"
                      />
            ...
            {/* Production Inputs */}
            <div>
              <span className="text-[10px] font-black text-[#5B60C4] uppercase tracking-wider block mb-1.5">生产产量</span>
              <div className="flex gap-2">
                <div className="flex-1 flex items-center gap-1.5 clay-tray rounded-2xl px-2.5 py-1">
                  ...
                  <Select value={item.batches.toString()} onValueChange={(v) => updatePerson(id, { batches: parseInt(v, 10) })}>
                    <SelectTrigger aria-label={`${item.name}的批数`} className="h-7 bg-transparent border-none text-xs font-black text-[#2D3142] px-1 focus:ring-0 shadow-none">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="rounded-2xl border-white shadow-xl bg-[#FAF7F2]">
                      {COUNTS.map((c) => (
                        <SelectItem key={c} value={c.toString()} className="font-bold text-xs">
                          {c} 批
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                <div className="flex-1 flex items-center gap-1.5 clay-tray rounded-2xl px-2.5 py-1">
                  ...
                  <Select value={item.pieces.toString()} onValueChange={(v) => updatePerson(id, { pieces: parseInt(v, 10) })}>
                    <SelectTrigger aria-label={`${item.name}的件数`} className="h-7 bg-transparent border-none text-xs font-black text-[#2D3142] px-1 focus:ring-0 shadow-none">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="rounded-2xl border-white shadow-xl bg-[#FAF7F2]">
                      {COUNTS.map((c) => (
            ...
            {/* Time Selectors */}
            <div>
              <span className="text-[10px] font-black text-[#5B60C4] uppercase tracking-wider block mb-1.5">工时记录</span>
              <div className="grid grid-cols-2 gap-2">
                <TimePicker value={item.startTime} onChange={(v) => updatePerson(id, { startTime: v })} label="上班" />
                <TimePicker value={item.endTime} onChange={(v) => updatePerson(id, { endTime: v })} label="下班" />
              </div>
            </div>
          </div>
        ) : (
          <div className="py-4 flex flex-col items-center justify-center bg-[#FAF6EE] rounded-2xl border border-white shadow-xs" aria-live="polite">
